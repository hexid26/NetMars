...
void RoutingTableV6::add(uint128_t addr, int len, uint16_t dest)
{
    ...
    m_Tables[len-1]->insert(masks(addr,len), dest);
}
    ...
    for (int i = 0; i < 128; i++){
        ...
        for (HashTable128::Iterator i = table->begin(); i != table->end(); ++i) {
            ...
            while (len_marker != len  && start <= end) {
                uint128_t temp = masks(*i, len_marker + 1);
                uint16_t marker_dest = lookup(&temp);
                if (len_marker < len) {
                    m_Tables[len_marker]->insert(masks(*i, len_marker +1), marker_dest, IPV6_HASHTABLE_MARKER);
                }
...
uint16_t RoutingTableV6::lookup(uint128_t *ip)
{
    ...
    do {
        ...
        uint16_t temp = m_Tables[len]->find(masks(*ip, len + 1));
