...
static void ac_build_goto(char *kws[], int n, ACMachine *acm) {
  int sid = 1;
  int i;
  ...
  for (i = 0; i < AC_ALPHABET_SIZE; i++) {
    if (states[0]->go.find(i) == states[0]->go.end()) states[0]->go[i] = 0;
  }
}

static void ac_build_failure(ACMachine *acm) {
  queue<int> q;
  ACState *s = acm->states[0];

  for (map<char, int>::iterator ite = s->go.begin(); ite != s->go.end(); ++ite) {
    if (ite->second != s->id) q.push(ite->second);
  ...
  while (!q.empty()) {
    ...
    q.pop();
    for (map<char, int>::iterator ite = r->go.begin(); ite != r->go.end(); ++ite) {
      q.push(ite->second);
      ...
      acm->states[ite->second]->output.insert(f->output.begin(), f->output.end());
    }
  }
}

static void ac_build_transition(ACMachine *acm) {
  queue<int> q;
  ACState *s = acm->states[0];

  for (int i = 0; i < AC_ALPHABET_SIZE; i++) {
    s->transition[i] = s->go[i];
    if (s->go[i] != s->id) q.push(s->go[i]);
  }
...
extern "C" int g4c_cpu_acm_match(g4c_acm_t *acm, uint8_t *data, int len) {
  int ret = 0, nid;
  uint8_t c;
...
extern "C" int ac_build_machine(ac_machine_t *acm, char **patterns, int npatterns, unsigned int memflags) {
  size_t psz = 0,  // total size of all pattern strings, including '\0'
  ...
  // calculate all sizes
  for (i = 0; i < npatterns; i++) psz += strlen(patterns[i]) + 1;
  ...
  if (acm->mem) {
    ...
    for (i = 0; i < acm->nstates; i++) {
      ...
      // copy output table
      for (set<int>::iterator ite = cpps->output.begin(); ite != cpps->output.end(); ++ite) {
        *tmpoutput = *ite;
        ++tmpoutput;
        ++output_offset;
      }
      ...
      memcpy(acm_state_transitions(acm, i), cpps->transition, sizeof(int) * AC_ALPHABET_SIZE);
    }
...
extern "C" void ac_release_machine(ac_machine_t *acm) { free(acm->mem); }

extern "C" int ac_match(char *str, int len, unsigned int *res, int once, ac_machine_t *acm) {
  int nm = 0;
    ...
    if (st->noutput > 0) {
      if (res) {
        for (int j = 0; j < st->noutput; j++) {
          ...
          res[ot] = 0x80000000 | (i + 1 - strlen(acm_pattern(acm, ot)));
        }
      }
      if (!nm && once) {
        return st->noutput;
...
static void dump_state(ACState *s, char *kws[]) {
...
static void dump_c_state(ac_state_t *s, ac_machine_t *acm) {
...
static void dump_c_acm(ac_machine_t *acm) {
  printf(
      "C version ACM:\n"
      ...
      acm->mem, acm->memsz, acm->memflags, acm->states, acm->nstates, sizeof(ac_state_t), acm->transitions,
      acm->outputs, acm->noutputs, acm->patterns, acm->npatterns);
  printf("States\n");
  for (int i = 0; i < acm->nstates; i++) dump_c_state(acm->states + i, acm);
}
