...
class IPsecAES {
 ...
 public:
  ...
  int process(int input_port, Packet *pkt) {
    ...
    cout << "需要加密的长度包含四个部分：原始的IP头、payload和padding以及extra，总长度encrypted_len为:" << encrypted_len
         << endl;
    ...
    if (anno_isset(&pkt->anno, NBA_ANNO_IPSEC_FLOW_ID)) {
      ...
      // }
      // cout << endl;
      cout << "正在采用AES加密结构中的AES_KEY和AES ctr128加密算法对原始的IP头、payload、padding、extra进行加密..."
           << endl;
