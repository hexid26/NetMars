 ...
 public:
  ...
  int process(int input_port, Packet *pkt) {
    ...
    unsigned char *payload_out =
        (unsigned char *)((uint8_t *)ethh + sizeof(struct ether_header) + sizeof(struct iphdr));
    int payload_len = (ntohs(iph->tot_len) - (iph->ihl * 4) - SHA_DIGEST_LENGTH);
    cout << "需要认证的部分包括ESP头、封装后的IP头（原始IP头）、payload、extra几个部分，长度为：" << payload_len
         << endl;
    ...
    for (int i = 0; i < 8; i++) *((uint64_t *)hmac_buf + i) = 0x3636363636363636LLU ^ *((uint64_t *)hmac_key + i);
    ...
    cout << "正在利用SHA1(安全散列算法1)生成hmac_buf前" << 64 + SHA_DIGEST_LENGTH << "个字节的散列值（消息摘要）"
         << endl;
    SHA1(hmac_buf, 64 + SHA_DIGEST_LENGTH, payload_out + payload_len);
    cout << "结果会保存在包的HMAC-SHA1 signature部分，并覆盖之前的内容，hash消息摘要为" << endl;
    for (int i = 0; i < SHA_DIGEST_LENGTH; i++) {
      cout << int(*(payload_out + payload_len + i)) << " ";
    }
    cout << endl;
