...
class IPsecESPencap {
 ...
 public:
  int process(int input_port, Packet *pkt) {
    ...
    cout << "拓展后IP数据报长度=带有ESP协议的新IP头长度+ESP头长度+IP数据报长度+padding长度+extra长度+HAMC-SHA1 "
            "signature长度="
         << extended_ip_len << "字节" << endl;
    ...
    // constexpr uintptr_t latency_offset = sizeof(struct ether_header) + sizeof(struct ipv4_hdr) + sizeof(struct
    // udphdr);
    static_assert(
        sizeof(struct udphdr) + sizeof(uint16_t) + sizeof(uint64_t) <= sizeof(struct esphdr) + sizeof(ipv4_hdr),
