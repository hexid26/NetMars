...
class PKT_Filter {
 public:
  ...
  inline uint16_t uint16_swap(uint16_t number);
...
std::string PKT_Filter::int_to_hex(int i) {
...
void PKT_Filter::print_MacInfo(char *buffer) {
...
// * 交换 uint16_t 的高低位
inline uint16_t PKT_Filter::uint16_swap(uint16_t number) { return number >> 8 | number << 8; }

// * 交换 uint32_t 的高低位
inline uint32_t PKT_Filter::uint32_swap(uint32_t number) {
  return (number << 24) | ((number & 0x00f0) << 8) | ((number & 0x0f00) >> 8) | (number >> 24);
}

// * 打印 UDP 包的信息
void PKT_Filter::print_UDPInfo(char *buffer) {
...
// * 打印 TCP 包的信息
void PKT_Filter::print_TCPInfo(char *buffer, uint16_t tcp_frame_length) {
  ...
  printf(
      " TCP frame INFO :: port %u to %u, data length = %u bytes, checksum = 0x%X, \n                   Sequence Number "
      "= %u, Acknowledgment Number = %u, WindowSize = %u, \n",
      uint16_swap(tcp_ptr->tcp_SrcPort), uint16_swap(tcp_ptr->tcp_DstPort), data_len,
      uint16_swap(tcp_ptr->tcp_CheckSum), uint32_swap(tcp_ptr->tcp_SequNum), uint32_swap(tcp_ptr->tcp_AcknowledgeNum),
      uint16_swap(tcp_ptr->tcp_WindowSize));
  printf(
      "                   URG_flag = %d, ACK_flag = %d, PSH_flag = %d, PST_flag = %d, SYN_flag = %d, FIN_flag = %d\n",
...
// * 打印 IP 包的信息
void PKT_Filter::print_IPInfo(char *buffer) {
  mac_frame_header *mac_ptr = (mac_frame_header *)buffer;
...
// 谭超 UDP & TCP print 函数
int PKT_Filter::print_UDPandTCP_Info(char *buffer) {
...
// * 快速甄别 TCP&UDP 数据报文
// * 返回值 0 都不是; 1 TCP; 2 UDP
int PKT_Filter::is_TCPorUDP(char *buffer) {
  if (*(unsigned short *)(buffer + 12) != 0x8) {
    return 0;
  }
